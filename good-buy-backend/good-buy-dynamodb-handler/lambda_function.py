...
def lambda_handler(event, context):
    ...
    if event['path'] == '/database/shopping-cart' and event['httpMethod'] == 'POST':
        ...
        get_action = DoesCartExist(event, db.Table("Shopping_Cart"))
        get_action.set_action()
        get_response = get_action.action()
        print(get_response)
        if "Item" not in get_response:
            ...
            'body': json.dumps(get_response, indent=4, default=str)
        }
...
class DoesCartExist(DBActionInterface):
    '''
    This class defines that will perform a GET action on the
    DynamoDB database. This class takes an event parameter and
    a table parameter. After action is performed it will
    ...
    def set_action(self):
        decoded_event = json.loads(self.event['body'])
        print("decoded event")
        print(decoded_event)
        id_value = decoded_event['ID']
        response = self.table.get_item(
            Key={
                'ID': id_value
            }
