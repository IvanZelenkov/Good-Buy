...
print ("Generating Shopping Cart...")


firstrun = True
lastrun = False
# Amount of Items randomly generated into cart
amountofproducts = 10
randomUserID = str(uuid.uuid4())
randomCartID = str(uuid.uuid4())
# Output File name
f = open("ExampleShoppingCart.json" , "w") 
# Local ID??
# Dont put into database tho

# If you HAVE an account, cart and user are the SAME
# If not they are different!?
#f.write("    \"CARTID\": \"" + randomCartID + "\",\n")


f.write("{\n")
f.write("  \"USERID\": \"" + randomUserID + "\", // This will be random unless linked to a user\n")
f.write("  \"ShoppingCart\": [\n")


# Loop for products in cart
for i in range(amountofproducts):
	...
	#write name
	f.write("		  \"Name\": \"" + randomname + "\",\n") # This needs to stay indented for whatever reason
	#write image url
